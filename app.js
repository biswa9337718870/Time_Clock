...
function setServerStatus(online) {
  ...
  if (online) {
    ...
    text.textContent = 'Database Server: Connected';
  } else {
    dot.className = 'geo-dot error';
    text.textContent = 'Database Server: Offline';
  }
}
...
async function autoStartAttendanceFlow() {
  ...
  const isBypassed = geo.ok === true || hasOutsideApproval;
...
function wireEventListeners() {
  ...
  $('geo-retry-btn').addEventListener('click', async () => {
    ...
    const isAllowed = geo.ok === true || hasOutsideApproval;
